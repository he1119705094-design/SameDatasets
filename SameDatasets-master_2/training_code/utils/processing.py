...
def make_processing(opt):
    ...
    # 添加图像块打乱变换（在ToTensor之后应用）
    if hasattr(opt, 'patch_size') and opt.patch_size > 0:
        try:
            from data_transforms import ImagePatchShuffle
            patch_shuffle = ImagePatchShuffle(patch_size=opt.patch_size, shuffle_prob=1.0)
            transforms_list.append(patch_shuffle)
            print(f"添加图像块打乱变换，patch_size={opt.patch_size}")
        except ImportError:
            print("警告：无法导入ImagePatchShuffle，跳过图像块打乱变换")
